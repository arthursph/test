...
Result.count({ $where: 'this.answers.length == 25' }).then(_count => {
    count = _count
    console.log(count)
    let cursor = Result.find({ $where: 'this.answers.length == 25' })
...
let calculateCo2e = result => {
    result.co2e = 0
    let answers = result.answers.sort((a, b) => {
...
let saveAnswer = (result, answer, i, nextAnswer) => {
    if (answer.answer) {
...
let calculateAnswerCo2e = (result, answer, i) => {
    switch (i) {
        case 2: {
            let multiplier = result.answers[0].answer.multiplier
            answer.multiplier[0] = answer.multiplier[1] / multiplier
            return answer
        }
        case 3: {
            let multiplier = result.answers[1].answer.multiplier[0]
            if (answer.index === 113) {
        ...
        case 5: {
            let $I$20 = result.answers[1].answer.multiplier[0]
            let $I$12 = result.answers[0].answer.multiplier
            let $I$25 = result.answers[2].answer.multiplier
            let $I$26 = result.answers[2].answer.co2e
        ...
        case 10: {
            answer.co2e =
                result.answers[8].answer.multiplier / answer.multiplier
            return answer
        }
        case 13: {
            if (answer.index === 219) {
        ...
        case 24: {
            answer.co2e =
                result.answers[22].answer.multiplier / answer.multiplier
            return answer
        }
        default: {
            return answer
